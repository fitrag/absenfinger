...
@php
    ...
    $canAccessPKL = $isAdmin; // Hanya Admin
    $canAccessGuruPKL = $isPKL; // Admin dan role PKL

    $isSiswa = $userLevel === 'siswa';
    $canAccessAbsenPKL = $isSiswa;
@endphp
    ...
    <div class="flex flex-col h-full">
        ...
        <nav class="flex-1 overflow-y-auto p-4 space-y-1">
                ...
                <div
                    ...
                    <div x-show="open" x-collapse class="mt-1 ml-4 pl-4 space-y-1 border-l border-slate-700/50">
                        ...
                        <div
                            ...
                            <div x-show="userOpen" x-collapse class="mt-1 ml-4 pl-4 space-y-1 border-l border-slate-700/50">
                                <!-- Users Guru -->
            ...
            @if($canAccessAdmGuru)
                ...
                <!-- PDS Guru (Dropdown) -->
                <div
                    x-data="{ open: {{ (request()->is('admin/guru*') && !request()->is('admin/guru-piket*') && !request()->is('admin/guru/pkl*')) || request()->is('admin/jurnal*') ? 'true' : 'false' }} }">
                    <button @click="open = !open"
                        class="w-full flex items-center justify-between gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 cursor-pointer
                                                                                                                                                                                                                                                                                      {{ (request()->is('admin/guru*') && !request()->is('admin/guru-piket*') && !request()->is('admin/guru/pkl*')) || request()->is('admin/jurnal*') ? 'bg-gradient-to-r from-blue-500/20 to-purple-500/20 text-white border border-blue-500/30' : 'text-slate-400 hover:text-white hover:bg-slate-800/50' }}">
                        <div class="flex items-center gap-3">
                            <svg class="w-5 h-5 {{ (request()->is('admin/guru*') && !request()->is('admin/guru-piket*') && !request()->is('admin/guru/pkl*')) || request()->is('admin/jurnal*') ? 'text-blue-400' : '' }}"
                                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                            </svg>
            ...
            @if($canAccessGuruPKL && !$canAccessPKL)
                <!-- Section: PKL (Guru) -->
                <div class="pt-4 pb-2">
                    <p class="px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">PKL</p>
                </div>
                ...
                <div x-data="{ open: {{ request()->is('admin/guru/pkl*') ? 'true' : 'false' }} }">
                    ...
                    <div x-show="open" x-collapse class="mt-1 ml-4 pl-4 space-y-1 border-l border-slate-700/50">
                        ...
                        <a href="{{ route('admin.guru.pkl.index') }}"
                            class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 cursor-pointer
                                                                                                                  {{ request()->is('admin/guru/pkl*') ? 'bg-blue-500/10 text-blue-400 border border-blue-500/20' : 'text-slate-400 hover:text-white hover:bg-slate-800/50' }}">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                            </svg>
        ...
        <div class="p-4 border-t border-slate-800/50">
            <div class="flex items-center gap-3 p-3 rounded-xl bg-slate-800/30">
                ...
                @endphp
                @if($userFoto)
                    <img src="{{ asset('storage/' . $userFoto) }}" alt="Profile"
                        class="w-10 h-12 rounded-lg object-cover shadow-lg shadow-purple-500/20">
                @else
                    <div
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        setTimeout(function () {
            const activeItem = document.querySelector('[class*="bg-blue-500/10"]');
            if (activeItem) {
