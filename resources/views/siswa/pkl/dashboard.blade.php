...
<div class="space-y-6" x-data="pklAttendance()">
    ...
    <div class="rounded-xl bg-slate-900/50 border border-slate-800/50 p-5">
        ...
        <div class="grid grid-cols-2 gap-4">
            <!-- Check In Status -->
            <div class="rounded-xl p-4 {{ $todayCheckIn ? 'bg-emerald-500/10 border border-emerald-500/30' : 'bg-slate-800/50 border border-slate-700/50' }}">
            ...
            <!-- Check Out Status -->
            <div class="rounded-xl p-4 {{ $todayCheckOut ? 'bg-blue-500/10 border border-blue-500/30' : 'bg-slate-800/50 border border-slate-700/50' }}">
                <div class="flex items-center gap-2 mb-2">
                    @if($todayCheckOut)
    ...
    <div class="space-y-3">
        ...
        @else
            <!-- Completed -->
            <div class="w-full py-4 px-6 bg-slate-800/50 border border-slate-700/50 text-slate-400 font-medium rounded-xl text-center">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="text-sm font-medium" 
                    :class="locationStatus === 'granted' ? 'text-emerald-400' : locationStatus === 'denied' ? 'text-red-400' : 'text-amber-400'"
                    x-text="locationStatus === 'granted' ? 'Lokasi Aktif' : locationStatus === 'denied' ? 'Lokasi Ditolak' : 'Menunggu Izin Lokasi'"></p>
    ...
    return {
        ...
        async doCheckIn() {
            ...
            try {
                ...
                const response = await fetch('{{ route("siswa.pkl.checkIn") }}', {
                    method: 'POST',
                    headers: {
        ...
        async doCheckOut() {
            ...
            try {
                ...
                const response = await fetch('{{ route("siswa.pkl.checkOut") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
