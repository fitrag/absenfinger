...
@section('title', 'Edit Surat Izin')

@section('page-title', 'Edit Surat Izin')
...
@section('content')
        ...
        <div class="bg-slate-900/50 backdrop-blur-xl border border-slate-800/50 rounded-2xl">
            ...
            <div class="px-6 py-4 border-b border-slate-700/50">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-lg font-bold text-white">Edit Surat Izin</h2>
                        <p class="text-slate-400 text-sm">Perbarui data surat izin Anda</p>
                    </div>
                </div>
            </div>

            <!-- Form -->
            <form action="{{ route('siswa.surat-izin.update', $suratIzin->id) }}" method="POST"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="tanggal_mulai"
                            value="{{ old('tanggal_mulai', $suratIzin->tanggal_mulai->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_selesai"
                            value="{{ old('tanggal_selesai', $suratIzin->tanggal_selesai->format('Y-m-d')) }}"
                            class="w-full px-4 py-2.5 bg-slate-800/50 border border-slate-700/50 rounded-xl text-slate-300 focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all text-sm">
                ...
                <div>
                    ...
                    <textarea name="keterangan" rows="4" placeholder="Jelaskan alasan izin Anda..."
                        class="w-full px-4 py-2.5 bg-slate-800/50 border border-slate-700/50 rounded-xl text-slate-300 focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all text-sm resize-none">{{ old('keterangan', $suratIzin->keterangan) }}</textarea>
                    @error('keterangan')
                        <p class="text-rose-400 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                @if($suratIzin->file_path)
                    <div class="flex items-center gap-3 p-3 bg-slate-800/50 rounded-xl">
                        ...
                        <div class="flex-1">
                            <p class="text-slate-300 text-sm font-medium">File Terlampir</p>
                            <a href="{{ route('siswa.surat-izin.download', $suratIzin->id) }}" target="_blank"
                                class="text-blue-400 text-xs hover:underline">Lihat File</a>
                        </div>
                    </div>
                @endif
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-300 mb-2">
                        {{ $suratIzin->file_path ? 'Ganti Lampiran (PDF)' : 'Lampiran (PDF)' }}
                    </label>
                    <div class="relative">
                        ...
                        <label for="file-input"
                            ...
                            <div class="text-center">
                                <p class="text-slate-300 text-sm font-medium" id="file-name">Klik untuk upload file PDF
                                    baru</p>
    ...
    <script>
        function updateFileName(input) {
            const fileName = input.files[0]?.name || 'Klik untuk upload file PDF baru';
