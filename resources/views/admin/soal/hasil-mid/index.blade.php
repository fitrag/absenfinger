...
@section('title', 'Hasil MID')
@section('content')
    <div class="space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-white">Hasil MID</h1>
                <p class="text-slate-400 text-sm mt-1">Lihat hasil ujian MID semester</p>
            </div>
        </div>
        ...
        <div class="rounded-xl bg-slate-800/50 border border-slate-700/50 overflow-hidden">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-slate-700/50">@forelse($ujians as $i => $u)<tr class="hover:bg-slate-800/30">
                    ...
                    <td class="px-4 py-3 text-sm text-slate-300">{{ $u->hasil_mids_count ?? $u->hasilMids->count() }}
                        siswa</td>
                    <td class="px-4 py-3 text-center"><a href="{{ route('admin.soal.hasil-mid.show', $u->id) }}"
                            class="px-3 py-1.5 bg-blue-500/20 text-blue-400 rounded-lg text-xs">Lihat Hasil</a></td>
                </tr>@empty<tr>
                        <td colspan="7" class="px-4 py-12 text-center text-slate-400">Belum ada hasil ujian MID</td>
                    </tr>@endforelse</tbody>
            </table>
            @if($ujians->hasPages())
