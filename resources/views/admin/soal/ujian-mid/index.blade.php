...
@section('title', 'Ujian MID')
@section('content')
    <div class="space-y-6" x-data="ujianMidPage()">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-white">Ujian MID</h1>
                <p class="text-slate-400 text-sm mt-1">Kelola jadwal ujian MID semester</p>
            </div>
            <button @click="showAddModal = true"
                class="inline-flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-blue-500 to-purple-500 text-white font-medium rounded-xl cursor-pointer"><svg
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-slate-700/50">@forelse($ujians as $i => $u)<tr class="hover:bg-slate-800/30">
                    ...
                    <td class="px-4 py-3 text-center"><button @click="openEditModal({{ json_encode($u) }})"
                        ...
                        <form action="{{ route('admin.soal.ujian-mid.destroy', $u->id) }}" method="POST" class="inline"
                        ...
                        <td colspan="7" class="px-4 py-12 text-center text-slate-400">Belum ada ujian MID</td>
                    </tr>@endforelse</tbody>
            </table>
            @if($ujians->hasPages())
        ...
        <div x-show="showAddModal" x-transition class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50"
            ...
            <div class="w-full max-w-2xl bg-slate-900 border border-slate-700/50 rounded-2xl">
                <div class="flex justify-between px-6 py-4 border-b border-slate-700/50">
                    <h3 class="text-lg font-bold text-white">Tambah Ujian MID</h3><button @click="showAddModal = false"
                        class="text-slate-400">&times;</button>
                </div>
                <form action="{{ route('admin.soal.ujian-mid.store') }}" method="POST" class="p-6 space-y-4">@csrf
                    ...
                    <div class="flex justify-end gap-3"><button type="button" @click="showAddModal = false"
                            ...
                            class="px-6 py-2 bg-blue-500 text-white rounded-xl">Simpan</button></div>
                </form>
            </div>
        </div>
        <div x-show="showEditModal" x-transition class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50"
            ...
            <div class="w-full max-w-2xl bg-slate-900 border border-slate-700/50 rounded-2xl">
                <div class="flex justify-between px-6 py-4 border-b border-slate-700/50">
                    <h3 class="text-lg font-bold text-white">Edit Ujian MID</h3><button @click="showEditModal = false"
                        class="text-slate-400">&times;</button>
                </div>
                <form :action="`{{ url('admin/soal/ujian-mid') }}/${editUjian.id}`" method="POST" class="p-6 space-y-4">
                    @csrf @method('PUT')
    ...
    <script>function ujianMidPage() { return { showAddModal: false, showEditModal: false, editUjian: {}, init() { this.$watch('showAddModal', v => document.body.classList.toggle('overflow-hidden', v)); this.$watch('showEditModal', v => document.body.classList.toggle('overflow-hidden', v)) }, openEditModal(u) { this.editUjian = { ...u, tanggal: u.tanggal ? u.tanggal.split('T')[0] : '' }; this.showEditModal = true } } }</script>
    @if(session('success'))
