...
@section('content')
    <div class="space-y-6">
        ...
        <div class="rounded-xl bg-slate-800/50 border border-slate-700/50 overflow-hidden">
            <form action="{{ route('admin.soal.hasil-us.bulkStore') }}" method="POST">@csrf<input type="hidden"
                    name="ujian_us_id" value="{{ $ujian->id }}">
                <table class="w-full">
                    <thead>
                ...
                <div class="px-4 py-3 border-t border-slate-700/50 flex justify-end"><button type="submit"
                        class="px-6 py-2 bg-purple-500 text-white rounded-xl">Simpan Nilai</button></div>
            </form>
        </div>
    </div>
    @if(session('success'))
