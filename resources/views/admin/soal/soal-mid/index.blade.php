...
@section('title', 'Soal MID')

@section('content')
    <div class="space-y-6" x-data="soalMidPage()">
        <!-- Header -->
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-white">Soal MID</h1>
                <p class="text-slate-400 text-sm mt-1">Kelola bank soal untuk ujian MID semester</p>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-700/50">
                        @forelse($soals as $index => $soal)
                            <tr class="hover:bg-slate-800/30 transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    <span
                                        class="px-2 py-1 text-xs font-medium rounded-lg {{ $soal->tipe_soal == 'pilihan_ganda' ? 'bg-blue-500/20 text-blue-300' : 'bg-amber-500/20 text-amber-300' }}">
                                ...
                                <td class="px-4 py-3 text-center">
                                    <div class="flex items-center justify-center gap-1">
                                        ...
                                        <form action="{{ route('admin.soal.soal-mid.destroy', $soal->id) }}" method="POST"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-12 text-center">
                                    ...
                                    <p class="text-slate-400">Belum ada soal MID</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div x-show="showAddModal" x-transition
            ...
            <div
                ...
                <div class="flex items-center justify-between px-6 py-4 border-b border-slate-700/50">
                    <h3 class="text-lg font-bold text-white">Tambah Soal MID</h3>
                ...
                <form action="{{ route('admin.soal.soal-mid.store') }}" method="POST" class="p-6 space-y-4">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-300 mb-2">TP <span
                                    class="text-red-400">*</span></label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-300 mb-2">Tipe Soal <span
                                    class="text-red-400">*</span></label>
                    ...
                    <div x-show="tipeSoal === 'pilihan_ganda'" class="space-y-3">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div>
                                <label class="block text-sm font-medium text-slate-300 mb-1">Opsi A</label>
                                <input type="text" name="opsi_a"
                                    class="w-full px-4 py-2 bg-slate-800/50 border border-slate-700/50 rounded-lg text-white text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-300 mb-1">Opsi B</label>
                                <input type="text" name="opsi_b"
                                    class="w-full px-4 py-2 bg-slate-800/50 border border-slate-700/50 rounded-lg text-white text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-300 mb-1">Opsi C</label>
                                <input type="text" name="opsi_c"
                                    class="w-full px-4 py-2 bg-slate-800/50 border border-slate-700/50 rounded-lg text-white text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-300 mb-1">Opsi D</label>
                                <input type="text" name="opsi_d"
                                    class="w-full px-4 py-2 bg-slate-800/50 border border-slate-700/50 rounded-lg text-white text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-300 mb-1">Opsi E</label>
                                <input type="text" name="opsi_e"
                                    class="w-full px-4 py-2 bg-slate-800/50 border border-slate-700/50 rounded-lg text-white text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-300 mb-1">Jawaban Benar</label>
        ...
        <div x-show="showEditModal" x-transition
            ...
            <div
                class="w-full max-w-3xl bg-gradient-to-br from-slate-900/95 to-slate-800/95 border border-slate-700/50 rounded-2xl shadow-2xl">
                <div class="flex items-center justify-between px-6 py-4 border-b border-slate-700/50">
                    <h3 class="text-lg font-bold text-white">Edit Soal MID</h3>
                ...
                <form :action="`{{ url('admin/soal/soal-mid') }}/${editSoal.id}`" method="POST" class="p-6 space-y-4">
                    @csrf
                    @method('PUT')
                    <div>
                        <label class="block text-sm font-medium text-slate-300 mb-2">Tipe Soal</label>
                        <select name="tipe_soal" x-model="editSoal.tipe_soal"
                            class="w-full px-4 py-2.5 bg-slate-800/50 border border-slate-700/50 rounded-xl text-white text-sm">
                            <option value="pilihan_ganda">Pilihan Ganda</option>
                            <option value="essay">Essay</option>
                        </select>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-300 mb-2">Pertanyaan</label>
                        <textarea name="pertanyaan" x-model="editSoal.pertanyaan" required rows="3"
                            class="w-full px-4 py-2.5 bg-slate-800/50 border border-slate-700/50 rounded-xl text-white text-sm"></textarea>
                    </div>
                    <div x-show="editSoal.tipe_soal === 'pilihan_ganda'" class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                            <label class="block text-sm font-medium text-slate-300 mb-1">Opsi A</label>
                            <input type="text" name="opsi_a" x-model="editSoal.opsi_a"
                                class="w-full px-4 py-2 bg-slate-800/50 border border-slate-700/50 rounded-lg text-white text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-300 mb-1">Opsi B</label>
                            <input type="text" name="opsi_b" x-model="editSoal.opsi_b"
                                class="w-full px-4 py-2 bg-slate-800/50 border border-slate-700/50 rounded-lg text-white text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-300 mb-1">Opsi C</label>
                            <input type="text" name="opsi_c" x-model="editSoal.opsi_c"
                                class="w-full px-4 py-2 bg-slate-800/50 border border-slate-700/50 rounded-lg text-white text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-300 mb-1">Opsi D</label>
                            <input type="text" name="opsi_d" x-model="editSoal.opsi_d"
                                class="w-full px-4 py-2 bg-slate-800/50 border border-slate-700/50 rounded-lg text-white text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-300 mb-1">Opsi E</label>
                            <input type="text" name="opsi_e" x-model="editSoal.opsi_e"
                                class="w-full px-4 py-2 bg-slate-800/50 border border-slate-700/50 rounded-lg text-white text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-300 mb-1">Jawaban Benar</label>
                            <select name="jawaban_benar" x-model="editSoal.jawaban_benar"
                                class="w-full px-4 py-2 bg-slate-800/50 border border-slate-700/50 rounded-lg text-white text-sm">
                                <option value="">Pilih</option>
                                <option value="A">A</option>
                                <option value="B">B</option>
    ...
    <script>
        function soalMidPage() {
            return {
                ...
                editSoal: {},
                init() {
                    this.$watch('showAddModal', value => document.body.classList.toggle('overflow-hidden', value));
                    this.$watch('showEditModal', value => document.body.classList.toggle('overflow-hidden', value));
                },
                openEditModal(soal) {
                    this.editSoal = { ...soal };
