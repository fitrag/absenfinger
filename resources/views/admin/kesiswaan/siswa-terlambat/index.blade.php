...
@section('content')
    <div class="space-y-6" x-data="siswaTerlambatPage()">
        ...
        <div class="rounded-xl bg-slate-900/50 border border-slate-800/50 p-4">
            <form action="{{ route('admin.kesiswaan.siswa-terlambat.index') }}" method="GET"
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b border-slate-800/50">
                            ...
                            <th class="px-4 py-3 text-center text-xs font-semibold text-slate-400 uppercase tracking-wider">Jam Datang</th>
                            <th class="px-4 py-3 text-center text-xs font-semibold text-slate-400 uppercase tracking-wider">Terlambat</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Alasan</th>
                    ...
                    <tbody class="divide-y divide-slate-800/50">
                        @forelse($siswaTerlambat as $index => $item)
                            <tr class="hover:bg-slate-800/30 transition-colors">
                                ...
                                <td class="px-4 py-3 text-sm text-center text-slate-300">{{ \Carbon\Carbon::parse($item->jam_datang)->format('H:i') }}</td>
                                ...
                                <td class="px-4 py-3 text-center">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border {{ $item->status_badge }}">
                                        {{ $item->status_label }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form action="{{ route('admin.kesiswaan.siswa-terlambat.destroy', $item) }}" method="POST" class="inline"
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" name="tanggal" required x-model="selectedTanggal" @change="loadLateStudents()"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-slate-300 mb-2">
                                    ...
                                    <span class="text-slate-500 text-xs ml-1" x-show="!loading && lateStudents.length > 0" x-text="'(' + lateStudents.length + ' siswa)'"></span>
                                </label>
                                <select name="student_id" required x-model="selectedStudent"
                                    class="w-full px-4 py-2.5 bg-slate-800/50 border border-slate-700/50 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500/50">
                                    <option value="">-- Pilih Siswa --</option>
                                    <template x-for="student in lateStudents" :key="student.id">
                                        <option :value="student.id" x-text="student.nis + ' - ' + student.name + ' (' + student.kelas + ')'"></option>
                                    </template>
                                </select>
                            </div>
                        </div>
                        <p class="text-xs text-slate-500 -mt-2" x-show="lateStudents.length === 0 && !loading && selectedTanggal">
                ...
                <form :action="'/admin/kesiswaan/siswa-terlambat/' + editData.id" method="POST">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-slate-300 mb-2">Jam Masuk Seharusnya *</label>
                                <input type="time" name="jam_masuk_seharusnya" required x-model="editData.jam_masuk_seharusnya"
                                    class="w-full px-4 py-2.5 bg-slate-800/50 border border-slate-700/50 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500/50">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-300 mb-2">Jam Datang *</label>
