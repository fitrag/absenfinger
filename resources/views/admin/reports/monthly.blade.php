...
@section('title', 'Laporan Per-Periode Bulan')
@section('page-title', 'Laporan Per-Periode Bulan')
...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-xl font-bold text-white">Laporan Presensi Per-Periode Bulan</h2>
        ...
        <div class="rounded-xl bg-slate-900/50 border border-slate-800/50 p-4 print:hidden">
            <form id="filterForm" action="{{ route('admin.reports.monthly') }}" method="GET" class="flex flex-wrap items-center gap-4">
                <div>
                    <label class="block text-xs text-slate-400 mb-1">Dari Bulan</label>
                    <input type="month" name="start_month" value="{{ $startMonth }}" onchange="document.getElementById('filterForm').submit()"
                ...
                <div>
                    <label class="block text-xs text-slate-400 mb-1">Sampai Bulan</label>
                    <input type="month" name="end_month" value="{{ $endMonth }}" onchange="document.getElementById('filterForm').submit()"
                        class="px-4 py-2 bg-slate-800/50 border border-slate-700/50 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500/50">
                </div>
                <div>
        ...
        <div class="hidden print:block mb-4">
            <h1 class="text-xl font-bold text-center">LAPORAN PRESENSI PER-PERIODE BULAN</h1>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-slate-800/50 print:border-black print:bg-gray-100">
                            <th
                                class="px-3 py-2 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider print:text-black">
                                No</th>
                            <th
                                class="px-3 py-2 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider print:text-black">
                                NIS</th>
                            <th
                                class="px-3 py-2 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider print:text-black min-w-[150px]">
                                Nama Siswa</th>
                            <th
                                class="px-3 py-2 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider print:text-black">
                                Kelas</th>
                            <th
                                class="px-3 py-2 text-center text-xs font-semibold text-blue-400 uppercase tracking-wider print:text-black bg-blue-500/10 print:bg-gray-200">
                                H%</th>
                            <th
                                class="px-3 py-2 text-center text-xs font-semibold text-purple-400 uppercase tracking-wider print:text-black">
                                S</th>
                            <th
                                class="px-3 py-2 text-center text-xs font-semibold text-cyan-400 uppercase tracking-wider print:text-black">
                                I</th>
                            <th
                                class="px-3 py-2 text-center text-xs font-semibold text-slate-400 uppercase tracking-wider print:text-black">
                                A</th>
                            <th
                                class="px-3 py-2 text-center text-xs font-semibold text-rose-400 uppercase tracking-wider print:text-black">
                                B</th>
                            <th
                                class="px-3 py-2 text-center text-xs font-semibold text-orange-400 uppercase tracking-wider print:text-black bg-orange-500/10 print:bg-gray-200">
                                Total</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-800/50 print:divide-gray-300">
                        @forelse($students as $index => $student)
                            ...
                            <tr class="hover:bg-slate-800/30 transition-colors print:hover:bg-transparent">
                                <td class="px-3 py-2 text-slate-400 print:text-black">{{ $index + 1 }}</td>
                                <td class="px-3 py-2 text-slate-300 font-mono print:text-black text-xs">{{ $student->nis }}</td>
                                <td class="px-3 py-2 text-white font-medium print:text-black">{{ $student->name }}</td>
                                <td class="px-3 py-2 text-slate-400 print:text-black">{{ $student->kelas->nm_kls ?? '-' }}</td>
                                <td
                                    class="px-3 py-2 text-center font-bold text-blue-400 print:text-black bg-blue-500/5 print:bg-gray-50">
                                    {{ $summary['percentage'] }}%</td>
                                <td class="px-3 py-2 text-center font-medium text-purple-400 print:text-black">
                                    {{ $summary['sakit'] }}</td>
                                <td class="px-3 py-2 text-center font-medium text-cyan-400 print:text-black">
                                    {{ $summary['izin'] }}</td>
                                <td class="px-3 py-2 text-center font-medium text-slate-400 print:text-black">
                                    {{ $summary['alpha'] }}</td>
                                <td class="px-3 py-2 text-center font-medium text-rose-400 print:text-black">
                                    {{ $summary['bolos'] }}</td>
                                <td
                                    class="px-3 py-2 text-center font-bold text-orange-400 print:text-black bg-orange-500/5 print:bg-gray-50">
                                    {{ $summary['total_absent'] }}</td>
                            </tr>
                        @empty
                            <tr>
        ...
        <div class="hidden print:block mt-8">
            <div class="flex justify-between items-start">
                <div class="text-sm">
                    <p>Keterangan:</p>
                    <p>H% = Persentase Kehadiran (dihitung dari tanggal yang ada data absensi)</p>
    ...
    <div id="pdfModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden items-center justify-center">
        <div class="bg-slate-900 rounded-2xl border border-slate-800 p-6 w-full max-w-2xl m-4">
            <div class="flex items-center justify-between mb-4">
                ...
                <button onclick="closePdfModal()" class="text-slate-400 hover:text-white cursor-pointer">
